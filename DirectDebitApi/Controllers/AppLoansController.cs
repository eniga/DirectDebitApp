...
namespace DirectDebitApi.Controllers
{
    ...
    public class AppLoansController : Controller
    {
        private IAppLoanService service;
        private ILogger<AppLoansController> logger;

        public AppLoansController(IAppLoanService service, ILogger<AppLoansController> logger)
        {
        ...
        // GET: api/values
        [HttpGet]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(IEnumerable<AppLoans>))]
        ...
        // GET api/values/5
        [HttpGet("{id}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(AppLoans))]
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(Response))]
        public async Task<ActionResult> PostAsync([FromBody] AppLoans item)
        {
            if (!ModelState.IsValid)
                return BadRequest();
            try
            {
                var exist = await service.GetAsync(x => x.loanid == item.loanid);
                if (exist != null)
                    return Conflict(new Response() { Status = false, Description = "Duplicate record" });
                var result = service.AddAsync(item).Result;
                if (result)
                {
                    var newitem = service.GetAsync(x => x.loanid == item.loanid).Result;
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(Response))]
        public async Task<ActionResult> PutAsync(int id, [FromBody] AppLoans item)
        {
