...
namespace DirectDebitApi.Controllers
{
    ...
    public class AppNotificationsTemplatesController : Controller
    {
        ...
        private ILogger<AppNotificationsTemplatesController> logger;

        public AppNotificationsTemplatesController(IAppNotificationsTemplateService service, ILogger<AppNotificationsTemplatesController> logger)
        {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(Response))]
        public async Task<ActionResult> PostAsync([FromBody] AppNotificatonsTemplate item)
        {
            if (!ModelState.IsValid)
                return BadRequest();
            try
            {
                var exist = await service.GetAsync(x => x.emailsubject == item.emailsubject);
                if (exist != null)
                    return Conflict(new Response() { Status = false, Description = "Duplicate record" });
                var result = await service.AddAsync(item);
                if (result)
                {
                    var newitem = await service.GetAsync(x => x.emailsubject == item.emailsubject);
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(Response))]
        public async Task<ActionResult> PutAsync(int id, [FromBody] AppNotificatonsTemplate item)
        {
