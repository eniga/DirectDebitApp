...
namespace DirectDebitApi.Controllers
{
    ...
    public class MerchantsController : Controller
    {
        ...
        private ILogger<MerchantsController> logger;

        public MerchantsController(IMerchantService service, ILogger<MerchantsController> logger)
        {
        ...
        // GET: api/values
        [HttpGet]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(IEnumerable<Merchants>))]
        ...
        // POST api/values
        [HttpPost]
        [ProducesResponseType(StatusCodes.Status201Created, Type = typeof(Merchants))]
        [ProducesResponseType(StatusCodes.Status409Conflict, Type = typeof(Response))]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(Response))]
        public async Task<ActionResult> PostAsync([FromBody] Merchants item)
        {
            if (!ModelState.IsValid)
                return BadRequest();
            try
            {
                var exist = await service.GetAsync(x => x.merchantname == item.merchantname);
                if (exist != null)
                    return Conflict(new Response() { Status = false, Description = "Duplicate record" });
                var result = await service.AddAsync(item);
                if (result)
                {
                    var newitem = await service.GetAsync(x => x.merchantname == item.merchantname);
        ...
        // PUT api/values/5
        [HttpPut("{id}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(Merchants))]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(Response))]
        public async Task<ActionResult> PutAsync(int id, [FromBody] Merchants item)
        {
