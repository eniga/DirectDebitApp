...
namespace DirectDebitApi.Controllers
{
    ...
    public class AppSettingsPreferencesController : Controller
    {
        private IAppSettingsPreferenceService service;
        private ILogger<AppSettingsPreferencesController> logger;

        public AppSettingsPreferencesController(IAppSettingsPreferenceService service, ILogger<AppSettingsPreferencesController> logger)
        {
        ...
        // GET: api/values
        [HttpGet]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(IEnumerable<AppSettingsPreferences>))]
        ...
        // GET api/values/5
        [HttpGet("{id}")]
        [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(AppSettingsPreferences))]
        ...
        // POST api/values
        [HttpPost]
        [ProducesResponseType(StatusCodes.Status201Created, Type = typeof(AppSettingsPreferences))]
        [ProducesResponseType(StatusCodes.Status409Conflict, Type = typeof(Response))]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(Response))]
        public async Task<ActionResult> PostAsync([FromBody] AppSettingsPreferences item)
        {
            if (!ModelState.IsValid)
                return BadRequest();
            try
            {
                var exist = await service.GetAsync(x => x.clientid == item.clientid);
                if (exist != null)
                    return Conflict(new Response() { Status = false, Description = "Duplicate record" });
                var result = await service.AddAsync(item);
                if (result)
                {
                    var newitem = await service.GetAsync(x => x.clientid == item.clientid);
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError, Type = typeof(Response))]
        public async Task<ActionResult> PutAsync(int id, [FromBody] AppSettingsPreferences item)
        {
